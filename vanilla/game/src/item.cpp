...
CItem::CItem(DWORD dwVnum)
	...
	m_bExchanging(false), m_pkDestroyEvent(NULL), m_pkExpireEvent(NULL), m_pkUniqueExpireEvent(NULL), m_pkTimerBasedOnWearExpireEvent(NULL),
...
bool CItem::AddToCharacter(LPCHARACTER ch, TItemPos Cell)
{
	...
	{
		if (m_wCell >= inventorysize && BELT_INVENTORY_SLOT_START > m_wCell)
		{
		...
		case ITEM_ROD:
			{
				if (bAdd)
				{
					if (m_wCell == inventorysize + WEAR_WEAPON)
...
bool CItem::EquipTo(LPCHARACTER ch, int bWearCell)
{
	if (!ch)
	{
	...
	m_pOwner = ch;
	m_bEquipped = true;
	m_wCell	= inventorysize + bWearCell;
...
bool CItem::Unequip()
{
	int inventorysize = INVENTORY_MAX_NUM;
	if(!bigger_inventory) inventorysize = 90;
	if (!m_pOwner || GetCell() < inventorysize)
	{
	...
	if (this != m_pOwner->GetWear(GetCell() - inventorysize))
	{
	...
	m_pOwner->SetWear(GetCell() - inventorysize, NULL);
...
void CItem::MoveToVoid()
{
	LPCHARACTER owner = GetOwner();
	if (owner == NULL)
	{
		sys_err ("MoveToVoid::Item %lu has no owner and therefore can't be moved into void. Sorry.", GetID());
		return;
	}
	SetWindow(VOID);
	owner->SetItem(TItemPos(m_bWindow, m_wCell), NULL);
	Save();
...
int CItem::GiveMoreTime_Per(float fPercent)
{
	if (IsDragonSoul())
	{
		...
		{
			SetSocket(ITEM_SOCKET_REMAIN_SEC, static_cast<signed long long>(duration));
