...
void CClientPackageCryptInfo::GetPackageCryptKeys( BYTE** ppData, int& iDataSize )
{
	...
	//NOTE : Crypt Key Info isnt updated during runtime. ( in case of file reloading all data is cleared & recreated )
	//its not safe but due to performance benefit we dont do re-serialize.
	if( m_pSerializedCryptKeyStream )
	{
