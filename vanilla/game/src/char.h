...
struct TSkillUseInfo
{
	...
	std::unordered_map<VID, size_t, std::hash<DWORD>> TargetVIDMap;
...
//		bool			IsPet() const		{ return m_bCharType == CHAR_TYPE_PET; }
		...
		BOOL 			IsGM() const;
		void			SetGMLevel(); 
		void			SetGMLevelForce(BYTE level) { m_pointsInstant.gm_level = level; }

		DWORD			GetExp() const		{ return m_points.exp;	}
		void			SetExp(DWORD exp)	{ m_points.exp = exp;	}
		bool			block_exp;  
		unsigned long long		GetNextExp() const;
	...
	protected:
		...
		std::vector<TCharacterMessage*> m_messages;
...
//		AttackLog			m_kAttackLog;
	...
	public:
		...
		/////messages
		DWORD				GetMessageCount() { return m_messages.size();}
		void				RecvMessage(std::string title, std::string msg, LPCHARACTER from, unsigned long long fee, std::vector<DWORD> items);
		TCharacterMessage*	ReadMessage(DWORD id);
		void				SaveMessages();
		bool				ReloadMessages();
		bool				DeleteMessage(DWORD id);
	...
	private:
		...
		bool bIsElevated;
		DWORD m_dwLastInterruptTime;
		std::vector<std::pair<std::string, std::string>> m_vTmpVar;

	public:
		void SetElevation(bool elevate) { bIsElevated = elevate; }
		bool GetElevation() { return bIsElevated; }
		DWORD GetLastInterruptTime() { return m_dwLastInterruptTime; }
		void SetLastInterruptTime(DWORD new_time) { m_dwLastInterruptTime = new_time; }
	...
	public:
		...
		void			SetTmpVar(std::string key, std::string var);
		void			DelTmpVar(std::string key);
		std::string			GetTmpVar(std::string key);
		bool			IsUnique() {return (UniqueLink ? true : false);}
		LPUNIQUE		GetUnique() {return UniqueLink;}
		void			SetUnique(LPUNIQUE unique) { UniqueLink = unique; }
};
