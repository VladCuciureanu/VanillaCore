...
{
	if (mysql_set_character_set(&m_hDB, "utf8"))
	{
...
bool CAsyncSQL::Connect()
{
	...
	if (!m_stLocale.empty())
	{
		...
		if (mysql_options(&m_hDB, MYSQL_SET_CHARSET_NAME, "utf8") != 0)
		{
			fprintf(stderr, "mysql_option failed : MYSQL_SET_CHARSET_NAME %s ", mysql_error(&m_hDB));
		}
	}

	fprintf(stderr, "Debug connect: %s, %s, %s, %s, %d, %lu\n", m_stHost.c_str(), m_stUser.c_str(), m_stPassword.c_str(), m_stDB.c_str(), m_iPort, CLIENT_MULTI_STATEMENTS);
